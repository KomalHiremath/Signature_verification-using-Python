...
def capture_image_from_cam_into_temp(sign=1):
    ...
    while True:
        ...
        if k % 256 == 27:  # ESC pressed
            ...
            img_name = f"{TEMP_FOLDER}/test_img{sign}.png"
...
def captureImage(ent, img_label, sign=1):
    filename = os.getcwd() + f'/test_img{sign}.png'
...
def checkSimilarity(window, path1, path2):
    if not validate_image(path1) or not validate_image(path2):
        messagebox.showerror("Error", "Both images must be from the 'temp' folder.")
        return
...
def validate_image(path):
    return TEMP_FOLDER in os.path.dirname(path)
